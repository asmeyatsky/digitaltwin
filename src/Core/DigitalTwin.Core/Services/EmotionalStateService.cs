    ...
    public class EmotionalStateService : IEmotionalStateService
    {
            ...
            try
            {
                ...
                var relevantMemories = allMemories
                    .Where(m => (m.Description != null && m.Description.Contains(context, StringComparison.OrdinalIgnoreCase)) ||
                               ...
                               (m.AssociatedEmotions != null && m.AssociatedEmotions.Any(e => e.ToString().Contains(context, StringComparison.OrdinalIgnoreCase))))
        ...
        public async Task<bool> UpdateMemoryImportanceAsync(Guid memoryId, Guid userId, int importance)
        {
            try
            {
                ...
                if (memory != null && memory.UserId == userKey)
                {
